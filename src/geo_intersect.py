...
def geo_intersect():
    ...
    logging.info("done processing building intersect geojson")
    logging.info('Time processed in building intersect: hh:mm:ss:', td)
...
def pop_intersect():
    ...
    popu_req = 'https://upri-noah.s3.ap-southeast-1.amazonaws.com/test/QC_FB_Bgy.geojson'
    popu_df = gpd.read_file(popu_req)
    popu_df = popu_df.to_crs('EPSG:32651')
    ...
    df_dict = []
    to_df = []
    for index, popu_row in popu_df.iterrows():
        p1 = popu_row.geometry
        store_dict = {}
        geom = []
        store_dict2 = {"Name":'', "Low Hazard":0,"Medium Hazard":0, "High Hazard":0, "All Hazard":0, "geometry": None}
        # print(popu_row.A1)
        a1 = popu_row.A1
        pop_2015 = popu_row.Pop2015
        for index2, haz_row in haz_df.iterrows():
            p2 = haz_row.geometry
            intersect = p1.intersects(p2)
            haz_lvl = haz_row.Var
            if intersect == True:
                ...
                area = round(area * 100) / 100
                union = gpd.GeoSeries(geom).unary_union
                store_dict2["Name"] = popu_row.Bgy_Name
                affected_formula = (area / a1) * pop_2015
                affected_formula = round(affected_formula / 1000000, 5)
                # affected_formula = round(affected_formula * 100) / 100
                print(popu_row.Bgy_Name, area, affected_formula)
                if haz_lvl == 1.0:
                    store_dict2["Low Hazard"] = affected_formula
                    store_dict2["geometry"] = union
                elif haz_lvl == 2.0:
                    store_dict2["Medium Hazard"] = affected_formula
                    store_dict2["geometry"] = union

                elif haz_lvl == 3.0:
                    store_dict2["High Hazard"] = affected_formula
                    store_dict2["geometry"] = union
                
                store_dict[popu_row.Bgy_Name] = store_dict2
                
        if store_dict:
            overall = store_dict[popu_row.Bgy_Name]['Low Hazard'] + store_dict[popu_row.Bgy_Name]['Medium Hazard'] + store_dict[popu_row.Bgy_Name]['High Hazard']
            store_dict[popu_row.Bgy_Name]['All Hazard'] = overall
            df_dict.append(store_dict[popu_row.Bgy_Name])
            polygon_geom2 = store_dict[popu_row.Bgy_Name]['geometry']
            to_df.append(polygon_geom2)
    end = (time.time() - start_time)

    td = datetime.timedelta(seconds=end)
    logging.info("done processing population intersect geojson")
    logging.info('Time processed in processing population intersect: hh:mm:ss:', td)
    df = pd.DataFrame(df_dict)
    gdf = gpd.GeoDataFrame(df, geometry=to_df)
    gdf = gdf.set_crs("EPSG:32651")
    gdf = gdf.to_crs("EPSG:4326")
    full_path = os.path.join(store_path, "popu_haz_intersect.geojson")
    gdf.to_file(full_path, driver='GeoJSON')
